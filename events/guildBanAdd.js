...
module.exports = class guildBanAdd extends Event {
	...
	async run(client, guild, user) {
		...
		if (logChannelId != undefined) {
			logChannel = client.guilds.cache.get(guild.id).channels.cache.get(logChannelId)
		...
		if (logModId != undefined) {
			logMod = client.guilds.cache.get(guild.id).channels.cache.get(logModId)
...
.user.id) return;
		...
		if (isOwner == true || guild.ownerID == action.executor.id || isOn == false) {
			...
			if (parseInt(count.get(action.executor.id)) < limit) {
				...
				let guild1 = client.guilds.cache.find(g => g.id === guild.id);
