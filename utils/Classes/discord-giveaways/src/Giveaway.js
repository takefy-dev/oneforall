...
class Giveaway extends EventEmitter {
    ...
    async checkWinnerEntry(user) {
        ...
        const hasPermissions = this.exemptPermissions.some((permission) => member.permissions.has(permission));
     ...
     */
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (winners.length > 0) {
                ...
                await this.message.edit(this.messages.giveawayEnded, { embeds : [embed] }).catch(() => {});
                ...
                this.message.edit(this.messages.giveawayEnded, { embeds : [embed] }).catch(() => {});
                resolve([]);
            }
        });
    }
    ...
    reroll(options) {
        return new Promise(async (resolve, reject) => {
            ...
            if (winners.length > 0) {
                ...
                await this.channel.send(options.messages.error);
                resolve([]);
            }
        });
    }
