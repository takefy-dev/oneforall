...
const main = module.exports = async (searchString, options, rt = 3) => {
  ...
  try {
    ...
    // Parse items
    resp.items = rawItems.map(a => PARSE_ITEM(a, resp)).filter(Boolean).filter((_, index) => index < opts.limit);
...
const parsePage2 = async (apiKey, token, context, opts) => {
  ...
  const parsedItems = rawItems.map(PARSE_ITEM).filter(Boolean).filter((_, index) => index < opts.limit);
